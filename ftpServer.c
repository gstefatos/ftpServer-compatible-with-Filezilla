...
while ((read_bytes = recv(client_fd, buffer, BUFFER_SIZE, 0)) > 0) {
    ...
    if (strcmp(cmd, "USER") == 0) {
        ...
        if (type == 'I') {
            ...
            snprintf(buffer, BUFFER_SIZE, "504 Command not implemented for that parameter.\r\n");
            send(client_fd, buffer, strlen(buffer), 0);
        }
    }
    ...
    else if(strcmp(cmd,"LIST") == 0)
    {
        ...
        if (dir == NULL) {
            ...
            snprintf(buffer, BUFFER_SIZE, "150 Opening ASCII mode data connection for file list.\r\n");
            send(client_fd, buffer, strlen(buffer), 0);
            ...
            if (stat(filepath, &file_stat) == 0) 
            {
                ...
                snprintf(entry_buffer, BUFFER_SIZE, "%s %ld %s %s %lld %s %s\r\n",
                            file_mode, (long)file_stat.st_nlink, user_info->pw_name, group_info->gr_name,
                            (long long)file_stat.st_size, time_buffer, entry->d_name);
    ...
    else if(strcmp(cmd,"MKD") == 0)
    {
        ...
        if(mkdir(finalDir,0777) == 0)response = "250 Directory created successfully.\r\n";
        else response = "550 Failed to create directory.\r\n";
